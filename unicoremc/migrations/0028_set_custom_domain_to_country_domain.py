    ...
    def forwards(self, orm):
        ...
        for project in orm.Project.objects.all():
            ...
            frontend_domain = "%(country)s.%(app_type)s.%(hub)s.unicore.io" % {
                ...
                'app_type': project.application_type.name,
                'hub': hub
            }
            project.frontend_custom_domain = ('%s %s' % (
                frontend_domain, project.frontend_custom_domain)).strip()
            project.cms_custom_domain = ('cms.%s %s' % (
                frontend_domain, project.cms_custom_domain)).strip()
            project.save()
