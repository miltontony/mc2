...
class NodeManager(object):
    ...
    def load_nginx_configs(self):
        if os.path.exists(self.nginx_path):
            self.nginx_configs = [
                Nginx(f, os.path.join(self.nginx_path, f))
                for f in os.listdir(self.nginx_path)
                if os.path.isfile(os.path.join(self.nginx_path, f)) and
                f.startswith('unicore_cms')
            ]
    ...
    def find_nginx(self, filename):
        if os.path.exists(os.path.join(self.nginx_path, filename)):
            for nginx in self.nginx_configs:
                if nginx.filename == filename:
                    return nginx
        return None
